...
import { useRegistionMutation } from "@/Redux/Feature/Auth/AuthApi";
...
const SignUp: React.FC<Props> = ({ setRoute }) => {
  ...
  const [registion, { isLoading, data, isSuccess, error }] =
  ...
  useEffect(() => {
    if (isSuccess) {
      const message = data?.message || "Registion SuccesFull";
      toast.success(message);
      setRoute("verifaction");
    } else if (error) {
      if ("data" in error) {
  ...
  return (
    <div className="w-full">
      <h1 className="title">Join to Lms</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center my-3 justify-center ">
          <FaGoogle className="cursor-pointer mr-2" size={30} />
